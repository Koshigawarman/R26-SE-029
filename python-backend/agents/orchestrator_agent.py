...
class OrchestratorAgent:
    ...
    def __init__(self, ollama_url: str, models: dict, max_retries: int = 3):
        ...
        # CriticAgent is the model-based diagnostic agent.
        critic_model = (
            models.get("critic")
            or models.get("debug")
            or models.get("codegen")
    ...
    def execute_stream(self, request: BuildRequest) -> Generator[str, None, None]:
        ...
        try:
            ...
            # ─────────────────────────────────────────────────────────────
            yield status("📁 STATE → CREATING_STRUCTURE: Creating project folders...", 15, "CREATING_STRUCTURE")
            ...
            # ─────────────────────────────────────────────────────────────
            # Phase 4: Debug → Memory → Critic → CodeGen → Retest Loop
            # ─────────────────────────────────────────────────────────────
            debug_success = False

            for attempt in range(1, self.max_retries + 1):
                debug_attempt_count = attempt
                progress_pct = 70 + int((attempt / self.max_retries) * 20)
                ...
                # If current test succeeds, store the previous successful correction case.
                ...
                latest_errors = [err.message for err in debug_result.errors]
